...
if($mailSatir.Count -gt 0){
  ...
  $govde = "DURUM:`n" + ($mailSatir -join "`n") + "`n`nNOT: Deterministik olanlar (Ithalat Rejimi/IGV/Damping) OTOMATIK guncellendi ve commit'lendi - sadece kontrol et. 'elle bak' yazanlar nuansli - birincil metni okuyup elle guncelle: KDV/OTV -> veri/gtip-*.json; CVOA tablosu -> veri/cvoa-oranlar.json (cok-oranli anlasmalarda GENEL orani al, ekipman/film alt-oranlarini not olarak yaz); GECIKME ZAMMI orani degistiyse -> ceza-asistani.html icindeki FAIZ_AY ve FAIZ_TARIH sabitlerini guncelle. Robot asla uydurmaz."
  try { & (Join-Path $PSScriptRoot 'alarm-maili.ps1') -Konu "KAYNAK NOBETCISI ($(Get-Date -Format dd.MM.yyyy)): degisiklik var" -Mesaj $govde } catch { Write-Host "MAIL hata: $($_.Exception.Message)" }
