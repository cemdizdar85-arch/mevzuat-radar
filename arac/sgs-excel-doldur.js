...
(async () => {
  ...
  const yeni = ['Son 7 dönemde (kez)', 'Eski soru kapı-temiz (birebir konu)', 'Kalıba ADAY (eski; kök eşleşmeli)', 'v29 basılan (07.09)', 'DURUM 07.09', 'Öncelik (dönem×sınav ağırlığı)'];
  ...
  for (let r = 2; r <= ws.rowCount; r++) {
    const row = ws.getRow(r); const ders = String(row.getCell(cDers).value || ''); const konu = katla(row.getCell(cKonu).value); if (!konu) continue;
    ...
    [pen, kk.temiz, kk.aday, v, durum, oncelik].forEach((val, i) => { row.getCell(c0 + i).value = val; });
