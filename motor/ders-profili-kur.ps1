...
foreach($svAd in @($prof.sinavlar.PSObject.Properties.Name)){
  ...
  foreach($dAd in @($prof.sinavlar.$svAd.PSObject.Properties.Name)){
    ...
    $tipik=@(); $aile=@()
    if($kisa){
      $kayitlar=@($tam | Where-Object { $_.sinav -eq $kisa -and ((Katla "$($_.bizim_ders)") -match [regex]::Escape($dKat) -or (Katla "$($_.arsiv_ders)") -match [regex]::Escape($dKat)) -and $_.donem -ge 1 })
    ...
    if(-not $dp.kapsam_tarifi){
      ...
      $dp | Add-Member -NotePropertyName _onay -NotePropertyValue 'BEKLIYOR' -Force
      $dolan++
    } else {
