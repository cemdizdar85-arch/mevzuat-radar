...
$etiketAdaySay=@{}; $etiketDersMap=@{}; $etiketKasaSay=@{}; $etiketTemizSay=@{}; $etiketDonemSay=@{}; $etiketBolumMap=@{}; foreach($e in $etiket.Keys){ $ka=(Katla2 $etiket[$e].ad); $etiketAdaySay[$ka]=$etiketAday[$e].Count; $etiketDersMap[$ka]=(EtiketDers $e); $etiketKasaSay[$ka]=$etiketKasa[$e]; $etiketTemizSay[$ka]=$etiketTemiz[$e]; $etiketDonemSay[$ka]=$etiket[$e].donemler.Count; $etiketBolumMap[$ka]="$($etiket[$e].bolum)" }   # 08.09: bölüm → plan üreticide ders uyumu süzgeci   # Excel doldurucu için: pencere etiketi (ASCII) -> kök eşleşmeli aday sayısı + bizim ders (07.09 Cem: köprü "Muhasebe" bölümünü toptan FMuh'a yazıyordu; * = sezgisel)
$jsYol=Join-Path $kok "veri\fabrika\eski-sgs-huni-$Tarih.json"; [IO.File]::WriteAllText($jsYol,(ConvertTo-Json -InputObject @{ tarih=(Get-Date -Format 'dd.MM.yyyy HH:mm'); dump=(Split-Path $Dump -Leaf); pencere=@($sonD | ForEach-Object { $_.donem }); ders=$tab; bosKonu=$bos; etiketAdaySay=$etiketAdaySay; etiketDers=$etiketDersMap; etiketKasaSay=$etiketKasaSay; etiketTemizSay=$etiketTemizSay; etiketDonemSay=$etiketDonemSay; etiketBolum=$etiketBolumMap; adayIdler=@($satir | Where-Object aday | ForEach-Object { [pscustomobject]@{ id=$_.id; ders=$_.ders; konu=$_.konu; hesapli=$_.hesapli } }) } -Depth 6 -Compress),[Text.UTF8Encoding]::new($false))
"yazildi: $mdYol · $jsYol"
