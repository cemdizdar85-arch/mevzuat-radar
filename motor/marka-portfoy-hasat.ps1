...
foreach($f in $firmalar){
  ...
  if($kritik.Count -gt 0 -and "$($f.kanal)" -eq 'mail' -and "$($f.email)" -match '^[^@\s]+@[^@\s]+\.[^@\s]+$'){
    ...
    $html  = "<p>Merhaba,</p><p><b>" + $unv + "</b> adına sicilde görünen markalardan " + $kritik.Count + " tanesinin yenileme tarihi yaklaştı:</p><ul>" + $satir + "</ul><p>Yenileme talebi koruma bitişinden önceki 6 ay içinde verilir; kaçırılırsa 6 ay ek süre + ek ücret (SMK m.23). Yenileme için <b>talep formu</b> ve <b>ücretin ödendiğini gösterir bilgi</b> verilir; bir taleple yalnız tek marka yenilenir, yenileme Bültende yayımlanır (SMK Uygulama Yönetmeliği m.18/1-3). Panelden bak: https://tetikte.com/marka-portfoy.html</p><p>Tetikte</p><p style='color:#888;font-size:12px'>Bu maili marka radarı aboneliğin için alıyorsun; kapatmak için bu maile 'iptal' yanıtı ver.</p>"
    if(MailAt $f.email ("Marka yenileme uyarısı - " + $unv) $html ("Merhaba,`n`n" + $unv + " adına markalarında yenileme tarihi yaklaşan " + $kritik.Count + " kayıt var:`n" + $duz + "`n`nYenileme talebi bitişten önceki 6 ayda verilir; kaçarsa 6 ay ek süre + ek ücret (SMK m.23). Talep formu + ücretin ödendiğini gösterir bilgi; bir taleple tek marka (SMK Uygulama Yönetmeliği m.18/1-3).`nPanel: https://tetikte.com/marka-portfoy.html`n`nTetikte`nKapatmak için bu maile 'iptal' yanıtı ver.")){ $mailAtilan++ }
  }
}
