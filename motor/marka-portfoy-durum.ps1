...
function SbGonder($yol,$metot,$govde,$ekBaslik){ $b=[Text.Encoding]::UTF8.GetBytes(($govde|ConvertTo-Json -Compress -Depth 6)); $bsl=$SB+@{'Content-Type'='application/json'}+$ekBaslik; $w=Invoke-WebRequest -Uri "$API_ADRES/$yol" -Method $metot -Headers $bsl -Body $b -UseBasicParsing -TimeoutSec 60 -SkipHttpErrorCheck; return [int]$w.StatusCode }
