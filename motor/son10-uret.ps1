...
$hedefler=@()
foreach($sv in 'SGS','SMMM','KGK'){ foreach($n in 1..10){ $hedefler+=('p90-{0}-{1:d2}-' -f $sv,$n) } }
...
foreach($sv in 'SGS','SMMM','KGK'){
  $sb=[Text.StringBuilder]::new()
  [void]$sb.Append("<!doctype html><html lang=""tr""><head><meta charset=""utf-8""><title>$($sinavAd[$sv]) — Son Kararla 10 Soru</title><style>$css</style></head><body>")
  [void]$sb.Append("<h1>$($sinavAd[$sv]) — en çok çıkan 10 konu, SON KARAR standardıyla</h1><p style='color:#aaa;font-size:13.5px'>29.08.2026 · Kural 25 (önce mantık + madde diyeti) + konsept şeması + çözüm tablosu. KASAYA YAZILMADI.</p>")
  $adet=0
...
"@)
  ...
  $hedef=Join-Path $SQLY ("son-karar-10-{0}.html" -f $sv)
  [IO.File]::WriteAllText($hedef,$sb.ToString(),[Text.UTF8Encoding]::new($false))
  "yazildi: son-karar-10-$sv.html ($adet soru)"
}
