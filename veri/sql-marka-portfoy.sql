...
create table if not exists public.marka_talep ( id bigint generated always as identity primary key, jeton text not null unique, unvan text not null, email text, durum text not null default 'bekliyor', sonuc jsonb, hata text, created_at timestamptz default now(), islendi_at timestamptz );
