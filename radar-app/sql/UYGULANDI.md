...
| `2026-09-25-uye-sayim.sql` | `public.uye_sayim()` — YALNIZ SAYI döner (toplam · son 1 saat · son 24 saat · son 1 saatin en yoğun dakikası); e-posta/kimlik dönmez; yalnız `service_role` çağırabilir | ⏳ **BASILMADI — 25.09.2026 yazıldı, Cem basacak.** Neden: saatlik üye alarmı (`uye-alarmi.yml`) bu fonksiyon olmadan **KÖR** — 24 saatte bir "sayım okunamıyor" maili atar. GM'nin tarayıcıdan basması güvenlik sınıflayıcısınca durduruldu (canlı veritabanına SQL). **Bastıktan sonra:** dosya sonundaki DOĞRULAMA a–c; sonra `powershell -NoProfile -File motor/uye-alarmi.ps1 -Kuru` YEŞİL demeli. Tablo değil fonksiyon, auth.users'a FK yok. |
