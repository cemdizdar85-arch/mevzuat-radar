...
| `veri/sql-form-kasasi.sql` | `form_kayit` tablosu (site formlarının kasası; anon/authenticated'a hak YOK, yalnız service_role yazar) | ⏳ **BASILMADI — Cem basacak (04.09.2026).** **Edge Function 04.09 akşam DEPLOY EDİLDİ — canlı adı `quick-task`** (Cem panelde isim kutusunu boş bıraktı, Supabase ad verdi; kod `radar-app/edge/form-al.ts`). "Verify JWT" AÇIK kaldı → 19 sayfa isteğe açık anahtarı `apikey`+`Authorization` başlığıyla ekler. Ölçüldü: `GET …/functions/v1/quick-task?tani=1` (apikey ile) → 200, `SERVICE_ROLE:true`, RESEND_KEY/RESEND_FROM/FORM_ALICI **false** = secrets henüz girilmedi. **Ölçüm:** aynı adres; ilk gerçek gönderide `kayit:true` dönmüyorsa tablo basılmamış. ⚠️ Secrets girilene kadar mail gitmez; tablo basılana kadar kayıt düşmez (ikisi de yoksa 502 → sayfada "iletilemedi"): `mrTeklif` formları ekranda "iletilemedi + mail adresi" gösterir, diğerleri kendi hata satırını gösterir. web3forms 04.09'da tüm sayfalardan çıkarıldı (Cem: "müşteri verisi tanımadığımız aracıdan geçmez"). |
