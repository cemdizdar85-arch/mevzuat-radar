...
| `veri/sql-marka-bulten-sahip.sql` | `ix_mb_sahipnorm` (trigram GIN) + `marka_norm_sozcuk()` + `marka_bulten_sahip(p_unvan,p_tavan)` **v3** (dönüşe `sahip` alanı; adres eşleşmesi elenir; **sözcük başı** eşleşme) | ✅ **v2 BASILDI 08.09.2026 06:30 (GM, Cem'in Chrome'undaki SQL Editor; Cem "sen basabilirsin"), canlı ölçüldü:** `EGE SERAMİK` 3,5 sn timeout → **0,7 sn 5 kayıt**, `ARÇELİK` 0,6 sn, `DİZDAR DENETİM` 0,17 sn, `sahip` alanı geliyor. ⏳ **v3 (sözcük başı) BASILMA SIRASINDA** — v2'de "ARÇELİK" → HEZAR ÇELİK KAPI, "EGE" → DEĞER geliyordu (boşluksuz alt dize). Eskitir: `veri/sql-marka-bulten.sql` içindeki `marka_bulten_sahip` (b). **v3 ölçümü:** `rpc/marka_bulten_sahip {"p_unvan":"ARÇELİK","p_tavan":500}` → satırlarda `sahip ilike '%hezar%'` 0 = v3 basılı; HEZAR varsa v2. 3 harfli arama (`EGE`) trigram daraltması zayıf olduğu için hâlâ zaman aşımına düşebilir; sayfa "ambar cevap vermedi" der. |
