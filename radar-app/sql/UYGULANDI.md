...
| `veri/sql-form-kasasi.sql` | `form_kayit` tablosu (site formlarının kasası; anon/authenticated'a hak YOK, yalnız service_role yazar) | ⏳ **BASILMADI — Cem basacak (04.09.2026).** Bununla birlikte **Edge Function `form-al`** de deploy bekliyor (`radar-app/edge/form-al.ts`, secrets: `RESEND_KEY` · `RESEND_FROM` · `FORM_ALICI`). **Ölçüm:** `GET …/functions/v1/form-al?tani=1` → 404 ise fonksiyon yok; 200 + `SERVICE_ROLE:true` ise deploy tamam; ilk gerçek gönderide `kayit:true` dönmüyorsa tablo basılmamış. ⚠️ **Fonksiyon deploy edilene kadar 19 sayfadaki formlar 404 alır**: `mrTeklif` formları ekranda "iletilemedi + mail adresi" gösterir, diğerleri kendi hata satırını gösterir. web3forms 04.09'da tüm sayfalardan çıkarıldı (Cem: "müşteri verisi tanımadığımız aracıdan geçmez"). |
