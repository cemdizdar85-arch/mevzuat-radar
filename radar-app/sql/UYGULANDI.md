...
| `2026-09-25-hesap-paylasim-korumasi.sql` | `uye_cihazlar` · `uye_ekran` · `uye_cihaz_olay` tabloları (RLS açık, politika YOK, auth.users'a FK YOK) · `cihaz_kontrol(p_cihaz,p_etiket,p_devral)` (tek ekran + 3 cihaz) · `cihazlarim(p_cihaz)` · `cihaz_cikar(p_hedef)` (30 günde 2) — üçü yalnız `authenticated` · **Eskitir:** `2026-09-25-uye-sayim.sql` içindeki `uye_sayim()` (+`paylasim_supheli`, `cihaz_siniri_24s`) | ⏳ **BASILMADI — 25.09.2026 yazıldı, Cem basacak.** İstemci (`cihaz-kapisi.js`, `paket-kapisi.js`, `ogrenci.html` Cihazlarım) canlıda ve **fail-open**: basılana kadar paketli üye normal girer (ölçüldü, prova 4/4), Cihazlarım "henüz açılmadı" der. **Bastıktan sonra:** dosya sonu DOĞRULAMA a–d + `scratchpad/paylasim-prova.js tam`. |
| `2026-09-25-uye-sayim.sql` | `public.uye_sayim()` — YALNIZ SAYI döner (toplam · son 1 saat · son 24 saat · son 1 saatin en yoğun dakikası); e-posta/kimlik dönmez; yalnız `service_role` çağırabilir | ✅ **BASILI — 25.09.2026, Cem SQL Editor'de çalıştırdı** (GM'nin tarayıcıdan basması güvenlik sınıflayıcısınca durdurulmuştu). **Doğrulandı (ölçüldü):** a) editörde `select public.uye_sayim()` → toplam 6, son 1 saat 0 · b) herkese açık anon anahtarla `POST /rest/v1/rpc/uye_sayim` → **HTTP 401** (yetkisiz; üye sayıları dışarı açık değil) · c) servis anahtarıyla `motor/uye-alarmi.ps1 -Kuru` → **YEŞİL**, sayılar okundu · d) bulut koşusu `uye-alarmi.yml` run 35794602663 → success, YEŞİL. Tablo değil fonksiyon, auth.users'a FK yok. |
| `2026-09-15-elci-programi.sql` | `siparisler` +3 kolon (`elci_kodu`, `indirim_tl`, `odendi_tarihi`) · `elciler` · `elci_indirim` (sgs=400) · `sinav_donemleri` · `elci_kodu_kontrol(p_kod,p_paket)` (anon çağırır, yalnız TL döner) · `siparis_elci_damga` + `siparis_odendi_damga` tetikleyicileri · `siparisler_ekle` politikası yeniden (elçi kodu biçimi eklendi) · `elci_komisyon(n,bk)` · `elci_donem_raporu` görünümü | ⏳ **BASILMADI — 15.09.2026 yazıldı, Cem basacak.** Neden: elçi davetinde "kodla 400 TL indirim" ve "750/1.000/1.250 komisyon" sözü verildi, sistemde karşılığı yoktu (mevcut davet kodu +1 ay veriyor, indirim yapmıyor). **Site tarafı hazır ama kapalı:** `fiyat-motoru.js` `ELCI.acik=false` → satin-al.html'de kod alanı görünmez, siparişe elçi kolonu gitmez. **Bastıktan sonra:** dosya sonundaki DOĞRULAMA a–d çalıştırılır; hepsi tutarsa `ELCI.acik=true` yapılır. DDL, auth.users'a FK YOK (14.09 kesinti tipi değil) ama yine düşük trafikte basılmalı. |
