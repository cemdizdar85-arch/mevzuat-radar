...
| `2026-09-25-uye-sayim.sql` | `public.uye_sayim()` — YALNIZ SAYI döner (toplam · son 1 saat · son 24 saat · son 1 saatin en yoğun dakikası); e-posta/kimlik dönmez; yalnız `service_role` çağırabilir | ✅ **BASILI — 25.09.2026, Cem SQL Editor'de çalıştırdı** (GM'nin tarayıcıdan basması güvenlik sınıflayıcısınca durdurulmuştu). **Doğrulandı (ölçüldü):** a) editörde  → toplam 6, son 1 saat 0 · b) herkese açık anon anahtarla  → **HTTP 401** (yetkisiz, üye sayıları dışarı açık değil) · c) servis anahtarıyla  → **YEŞİL**, sayılar okundu. Tablo değil fonksiyon, auth.users'a FK yok. |
