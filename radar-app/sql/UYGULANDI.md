...
| `2026-09-07-alacak-bos-dizi-onarimi.sql` | **Eskitir:** `2026-08-20-alacak-metin-alanlari.sql` içindeki `alacak_borclu_maskele` + `alacak_yaz`. Maskeleme null öğeyi atlar; yazma `[null]`/`[]` geldiğinde eskisini korur. Sebep: 07.09 gece yükleyici `[null]` gönderip 5.643 satırın borçlu dizisini ezdi, `alacak_ara` 400 verdi (veri PATCH + 04.09 yedeğiyle onarıldı, yükleyici düzeltildi) | ⏳ **BASILMADI — Cem basacak.** Ölçüm: `curl -s -X POST .../rpc/alacak_borclu_maskele -H 'apikey: <anon>' -H 'Content-Type: application/json' -d '{"p":[null]}'` → basılıysa `null`/200, basılmadıysa `22023 cannot delete from scalar` |
...
| `2026-09-08-kurulus-nobet.sql` | `kurulus_nobet` tablosu (Kuruluş Nöbeti "nöbete al": e-posta + tür + tescil tarihi + olay listesi; anon yalnız ekler, `kvkk=true` şart). Okuyan robot: `motor/kurulus-nobet-postacisi.ps1` (hoş geldin + 3 gün önce hatırlatma). | 🔴 **BASILMADI — Cem basacak.** Basılana kadar sayfa 404'ü sessizce yutar, yalnız quick-task maili (Cem'e) gider, kullanıcıya hatırlatma GİTMEZ; postacı robot KÖR (exit 2) + Cem'e mail. **Ölçüm:** anon `POST /rest/v1/kurulus_nobet` boş gövde → 400 (basılı) / 404 PGRST205 (basılmamış). |
...
| `veri/sql-marka-bulten-sahip.sql` | `ix_mb_sahipnorm` (trigram GIN) + `marka_bulten_sahip(p_unvan,p_tavan)` **v2** (dönüşe `sahip` alanı; adres eşleşmesi elenir) | ⏳ **BASILMADI — 08.09.2026 yazıldı, Cem basacak.** Ölçülen kusur: v1 gerçek unvanlarda (`EGE SERAMİK`, `ARÇELİK`, `DİZDAR DENETİM`) **57014 statement timeout** — `sahip_norm` üzerinde yalnız düz btree var, `'%…%'` taraması 799 bin satırı okuyor. Eskitir: `veri/sql-marka-bulten.sql` içindeki `marka_bulten_sahip` (b). **Ölçüm:** `rpc/marka_bulten_sahip {"p_unvan":"ARÇELİK","p_tavan":5}` → 200 + satırlarda `sahip` alanı = basılı; 57014 ya da `sahip` alanı yok = basılmamış. `marka-portfoy.html` basılmadan da çalışır: zaman aşımında "ambar cevap vermedi" der, "başvuru yok" DEMEZ. |
