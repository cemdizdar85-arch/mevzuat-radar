...
| `2026-09-06-cevap-kayit.sql` | `cevap_kayit` tablosu (anon yalnız ekler) + `sik_yuzdesi(p_soru_id)` (security definer, anon çağırır, yalnız şık sayımı döner) | ⏳ **BASILMADI — Cem Supabase SQL Editor'de bir kez çalıştıracak.** Basılana dek Kaydır-Çöz akran yüzdesi göstermez, POST 404 sessiz. Ölçüm: anon `rpc/sik_yuzdesi` → 200 / 404 `PGRST202`. |
