...
| `veri/sql-siparis.sql` | `siparisler` tablosu | ❌ **TABLO YOK** (`PGRST205`) — ödeme hattı beklediği için normal. ⚠️ 29.08 İKİ DÜZELTME: (1) kütükte tablo adı `siparis` yazıyordu, **doğrusu `siparisler`** — sayfa da SQL de `siparisler` diyor, kütük yanlış ölçmüş; (2) dosya o güne kadar **depoda değildi** (takipsiz), bu turda push edildi. Ayrıca politikası `with check (true)` idi (her içeriğe açık) → **durum/biçim kilidi eklendi**, en önemlisi dışarıdan gelen sipariş artık zorunlu `odeme_bekliyor` doğuyor. |
| `veri/sql-guvenlik-kapisi.sql` | `leadler` anon yazma izni | ❌ **BASILMADI** — 29.08 kuruldu. Ölçüm: `POST leadler {}` → `23502` (RLS yazmayı engellemedi, sadece NOT NULL durdurdu) = anonim sınırsız satır basılabilir. Kapatmanın maliyeti sıfır: `leadler`'e yazan hiçbir site kodu yok (formlar web3forms'a gidiyor). Basılınca beklenen: `42501`. |
